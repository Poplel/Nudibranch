...
an iOS Shortcut, or an in-app event), then **notifies** per its configured mode. Every run is ALWAYS
...
the webhook route, the worker scheduler tick, and worker event hooks.
"""
...
# Keep every value present in worker `TASK_HANDLERS` and every slug the clients can offer:
# an unknown slug only fails at fire time (`_run_tool` raises), so the automation saves fine
...
EVENT_TYPES = {"download_complete", "wishlist_match", "scan_complete", "podcast_episode_added"}
