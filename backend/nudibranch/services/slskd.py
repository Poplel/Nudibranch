...
def search_payload(client: httpx.Client, search_id: str) -> Any:
    ...
    if responses_payload:
        return {**state_payload, **responses_payload} if isinstance(state_payload, dict) else responses_payload
...
def _strip_accents(value: str) -> str:
    """Fold accented characters to their plain ASCII base letter (e.g. "Beyoncé" -> "Beyonce")
    rather than leaving them mismatched. A shared network routinely has the same release under
    both spellings — a filename tagged "Beyoncé" inside a folder literally named "Beyonce" is a
    real, confirmed case — and without this an accented query token never matches an unaccented
    filename (or vice versa), silently dropping a real candidate's relevance to 0."""
    return "".join(
...
def query_match_score(query: str, filename: str) -> int:
    haystack = " ".join(
        _strip_accents(str(filename or "").replace("\\", "/").replace("_", " ").replace("-", " "))
        .casefold()
        .split()
    )
    tokens = [
        token
        for token in _strip_accents(str(query or "")).casefold().replace("-", " ").split()
        if len(token) > 2
    ]
